...
export default function StudyScreen() {
    ...
    const correctSwipes = useRef(0);
    const flipUpdatesMap = useRef<Map<string, number>>(new Map());
    ...
    useEffect(() => {
        const fetchCardsAndTheme = async () => {
            try {
                const [cards, storedTheme] = await Promise.all([
                    id ? DeckService.getDeckById(id as string) : null,
    ...
    useEffect(() => {
        ...
        const percent = (Math.min(currentIndex + 1, flashcards.length) / flashcards.length) * 100;
    ...
    const finishStudySession = async () => {
        ...
        const minsSpent = Math.max(1, Math.round((endTime - startTime.current) / 60000));
        await StatsCacheService.addStudyTime(minsSpent, flashcards.length, correctSwipes.current);

        if (flipUpdatesMap.current.size > 0) {
            const updatesArray = Array.from(flipUpdatesMap.current.entries()).map(([cardId, currentFlipCount]) => ({
                cardId,
                flipCount: currentFlipCount + 1,
            }));

            try {
                await CardService.updateFlipCount(updatesArray);
                console.log("Đã cập nhật tiến độ thẻ thành công");
            } catch (error) {
                console.log("Lỗi cập nhật tiến độ thẻ:", error);
            }
        }
    };
    ...
    const resetDeck = () => {
        ...
        correctSwipes.current = 0;
        flipUpdatesMap.current.clear();
        progressWidth.value = 0;
    };

    const handleSwipe = (isRight: boolean) => {
        if (currentIndex >= flashcards.length || isSwiping.current) return;

        if (isRight) {
            correctSwipes.current += 1;
        } else {
            const cardId = flashcards[currentIndex].flashcardId;
            const currentFlip = flashcards[currentIndex].flipCount || 0;
            flipUpdatesMap.current.set(cardId, currentFlip);
        }
    ...
    const endScreenAnimatedStyle = useAnimatedStyle(() => ({ opacity: endScreenOpacity.value }));
    ...
    const nextWord = flashcards[currentIndex + 1];

    if (isLoading) {
        return (
            <LinearGradient
                colors={[currentTheme.customBackground as string, currentTheme.white]}
            ...
            >
                <SafeAreaView style={{ flex: 1, padding: 20 }}>
                    ...
                    <View style={{ flex: 1, justifyContent: "center", alignItems: "center" }}>
                        <Text style={{ color: currentTheme.subText, fontSize: 16 }}>
                            Bộ bài này chưa có thẻ nào. Hãy thêm thẻ trước nhé!
                        </Text>
                    </View>
                </SafeAreaView>
            </LinearGradient>
                        ...
                        <>
                            <View style={styles.cardContainer}>
                                {nextWord && (
                                    <Animated.View
                                        style={[
                                            styles.card,
                                            { backgroundColor: activeMode === "dark" ? "#1c2d5a" : "#92CEFF" },
                                    ...
                                    >
                                        <View style={styles.cardInnerFront}>
                                            ...
                                            <View style={styles.centerContent}>
                                                <Text style={[styles.cardWordFront, { color: currentTheme.mainText }]}>
                                                    {nextWord.word}
                                                </Text>
                                            </View>
                                            <Text style={[styles.cardHintFront, { color: currentTheme.mainText }]}>
                                                Tap to flip
                                            </Text>
                                        </View>
                                    </Animated.View>
                                ...
                                >
                                    ...
                                    <Animated.View
                                        style={[
                                            ...
                                            styles.cardBack,
                                            backAnimatedStyle,
                                            { backgroundColor: activeMode === "dark" ? "#2c3e6b" : "#3B548C" },
                                        ]}
                                    >
                                        <Animated.View style={overlayStyle} pointerEvents="none" />
                            ...
                            <View style={styles.evalContainer}>
                                <TouchableOpacity
                                    style={[
                                        styles.evalBtn,
                                        { backgroundColor: activeMode === "dark" ? "#5a2a2a" : "#FFE5E5" },
                                    ]}
                                    activeOpacity={0.7}
                                ...
                                <TouchableOpacity
                                    style={[
                                        styles.evalBtn,
                                        { backgroundColor: activeMode === "dark" ? "#2a5a2a" : "#E6FADC" },
                                    ]}
                                    activeOpacity={0.7}
...
const styles = StyleSheet.create({
    ...
    card: { width: width * 0.85, height: "100%", borderRadius: 24, backfaceVisibility: "hidden", position: "absolute" },
    cardBack: { top: 0 },
    ...
    cardWordFront: { fontSize: 34, fontWeight: "900", textAlign: "center" },
    cardHintFront: { fontSize: 16, opacity: 0.7, marginBottom: 10 },
